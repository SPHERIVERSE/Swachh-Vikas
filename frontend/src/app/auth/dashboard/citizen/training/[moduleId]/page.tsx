...
export default function CitizenModulePage() {
  ...
  const [userAnswer, setUserAnswer] = useState(''); // NEW STATE FOR SUBJECTIVE ANSWERS
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900">
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-12 text-center border border-white/20 shadow-2xl animate-fade-in">
          ...
          <p className="text-blue-200 text-sm mt-2">Unlocking your next quest</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 relative overflow-hidden">
          ...
          <div className="animate-fade-in-down">
            <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 border border-white/20 shadow-2xl">
              ...
              <h1 className="text-4xl font-bold text-white mb-4 text-center">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400">{moduleTitle}</span>
              </h1>
            ...
            <div className="animate-fade-in-up">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 text-center border border-white/20 shadow-2xl cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-3xl"
                  ...
                  <div className="text-3xl font-bold text-blue-400">{flashcards.length}</div>
                  <div className="text-sm text-blue-200">Cards Available</div>
                </div>
                ...
                <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 text-center border border-white/20 shadow-2xl cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-3xl"
                  ...
                  <div className="text-3xl font-bold text-green-400">{videos.length}</div>
                  <div className="text-sm text-green-200">Videos Available</div>
                </div>
                ...
                <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 text-center border border-white/20 shadow-2xl cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-3xl"
                  ...
                  <div className="text-3xl font-bold text-purple-400">{quizzes.length}</div>
                  <div className="text-sm text-purple-200">Quizzes Available</div>
                </div>
              </div>
            </div>
          )}
          ...
          {currentSection === 'flashcards' && flashcards.length > 0 && (
            <div className="animate-fade-in-up">
              <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 border border-white/20 shadow-2xl">
                ...
                {/* The Flashcard Container with the new conditional rendering and fade animation */}
                <div className="max-w-2xl mx-auto h-80 md:h-96 relative">
                  ...
                  <div className="flex justify-between items-center mt-6">
                    <button
                      onClick={(e) => {
                        e.stopPropagation(); // Prevent the parent card from flipping
              ...
              <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 border border-white/20 shadow-2xl">
                ...
                {!quizStarted && !showScore ? (
                  <div className="text-center max-w-md mx-auto">
                    ...
                    <button onClick={startQuiz} className="w-full py-4 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white rounded-xl font-bold text-lg transition-all duration-300 transform hover:scale-105 shadow-lg">
                      🚀 Start Quiz
                    </button>
                  </div>
                ) : showScore ? (
                  <div className="text-center max-w-md mx-auto">
                    ...
                    <div className="space-y-3">
                      <button
                        ...
                        className="w-full py-3 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white rounded-xl font-bold transition-all duration-300 transform hover:scale-105 shadow-lg"
                      >
                        🔄 Retry Quiz
                      </button>
                      <button
                  ...
                  <div className="max-w-2xl mx-auto animate-fade-in-up">
                    ...
                    <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 mb-6 border border-white/20 shadow-xl">
                      ...
                      {currentQ?.type === 'MCQ' ? (
                            ...
                            return (
                              <div
                                ...
                                className={`p-4 rounded-xl border transition-all duration-300 cursor-pointer ${
                                  submitted ? (option.isCorrect ? 'border-green-500/50 bg-green-900/20' : isSelected ? 'border-red-500/50 bg-red-900/20' : 'border-gray-500/20 bg-white/5') : isSelected ? 'border-blue-500/50 bg-blue-900/20' : 'border-gray-500/20 bg-white/5 hover:bg-white/10'
                                }`}
                                onClick={() => handleOptionSelect(option.id)}
                              >
                                <div className="flex items-center space-x-3">
                                  <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
                                    submitted ? (option.isCorrect ? 'border-green-500' : isSelected ? 'border-red-500' : 'border-gray-500') : isSelected ? 'border-blue-400' : 'border-gray-500'
                                  }`}>
                                    {submitted && (option.isCorrect || isSelected) ? <div className={`w-3 h-3 rounded-full ${option.isCorrect ? 'bg-green-500' : 'bg-red-500'}`}></div> : isSelected && <div className="w-3 h-3 bg-blue-400 rounded-full"></div>}
                                  </div>
                        ...
                        // NEW JSX FOR SUBJECTIVE QUESTIONS
                        <div>
                            <textarea
                                value={userAnswer}
                                ...
                                rows={5}
                                className={`w-full p-4 rounded-xl border border-white/20 bg-white/5 text-white placeholder-gray-400 outline-none transition-all duration-300 focus:border-purple-400 resize-none ${submitted ? 'cursor-not-allowed' : ''}`}
                            ></textarea>
                            {submitted && (
                    ...
                    <div className="text-center">
                      {!submitted ? (
                        <button
                          ...
                          className="w-full py-4 bg-gradient-to-r from-purple-500 to-pink-600 hover:from-purple-600 hover:to-pink-700 text-white rounded-xl font-bold text-lg transition-all duration-300 transform hover:scale-105 shadow-lg disabled:opacity-50 disabled:hover:scale-100"
                        >
                          Submit Answer
                        </button>
                      ) : (
        ...
        <style jsx>{`
          ...
          .confetti-rain::before {
            ...
            width: 100%;
            height: 100%;
            background: linear-gradient(135deg, #ff007f, #00ffff, #ff007f, #00ffff);
          ...
          .rotate-y-180 .flashcard-text-back {
            opacity: 1;
            transform: rotateY(180deg); /* Counter-rotate to prevent mirroring */
          }
        `}</style>
      </div>
    </RoleGuard>
